...
- E09 permissioned MCP tool profile design/schema/docs for visible-vs-callable tool boundaries; runtime profile enforcement is intentionally deferred to a later gate;
...
- v0.4.3-alpha final publication smoke, verifier, upgrade notes, manifest, and tag evidence for the MCP upstream enforcement gate;
...
PyPI packaging is not the supported v0.4.2-alpha distribution path. Install from a GitHub clone for now, because the router skills, scripts, docs, and bundled packs are repo assets. A PyPI package should wait until wheel/sdist asset inclusion and installer behavior are tested in CI.
