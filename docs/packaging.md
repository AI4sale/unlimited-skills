...
Status: stable `v0.6.9` PyPI-first release train.
...
python -m pip install --upgrade "unlimited-skills[all]>=0.6.9"  # vector + daemon
```
...
The v0.6.9 workflow builds wheel and sdist once, runs `twine check` and tests
...
See `docs/releases/v0.6.9-plan.md` and `.github/workflows/publish-pypi.yml` for the
exact contract.
