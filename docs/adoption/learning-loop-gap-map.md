...
Status: historical W1 handoff map for W2, updated after v0.6.3 #173/#175. This
document records what must exist before Unlimited Skills can claim a complete
non-dry-run local learning loop.
...
| Missed/wrong feedback | `feedback record` supports `missed` and shipped public `wrong`; support response pack remains available | No maintainer acceptance workflow for turning validated signals into eval cases | Retrieval owner | Add redacted feedback-to-eval candidate workflow | Use public/manual issue labels | W2 |
| Eval candidate | Frozen eval set and checker exist | No accepted maintainer workflow converts feedback reports into eval fixture drafts | Retrieval owner | Add maintainer-reviewed feedback-to-eval candidate builder | Manual eval PR with redacted evidence | W2 |
...
| Should wrong and missed reasons get sub-reason categories beyond the shipped verdicts? | Retrieval owner | Choose optional sub-reason taxonomy before implementation | Keep shipped `missed` / `wrong` verdicts only |
