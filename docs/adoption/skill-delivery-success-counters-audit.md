...
| use rejected / wrong skill | explicit local feedback | `feedback record <skill> --verdict rejected`; issue template `skill-not-invoked`; support response pack | per-skill rejected count, wrong-skill public/manual labels | automatic wrong-skill category from local CLI | local-only/manual; no auto-upload | partial | Codex | Count rejected verdicts and route wrong-skill reports | Use GitHub issue template when local feedback is insufficient | W1.1 add rejected_count and wrong_skill_report_count when local source exists |
| missed-skill report | manual public-alpha feedback | `skill-not-invoked` issue template; `feedback prepare`; support triage workflow | manual report category, reproduction owner, issue labels | local CLI counter for "expected skill but silence" | manual only; no prompt collection | partial | Retrieval owner | Convert validated misses into eval candidates | Use public issue labels when local counter is absent | W2 add missed-skill eval candidate creation path |
| wrong-skill report | manual/local feedback | rejected feedback verdict; `skill-not-invoked` issue template; support response pack | rejected count, manual wrong-suggestion label | structured wrong-skill reason in local event row | local/manual only | partial | Retrieval owner | Separate generic rejection from wrong-skill category | Ask reporter for redacted command/category, not raw prompt | W2 add wrong-skill reason taxonomy |
