...
`v0.3.1-alpha` is a developer preview stabilization train, not a stable production SLA. The published baseline is `v0.3.0-alpha`.
...
- PyPI is not the supported v0.3.1-alpha distribution path because router skills, scripts, docs, and bundled packs are repo assets.
