...
- **UserPromptSubmit hook** (`plugin/hooks/user_prompt_submit.py`): runs the `suggest` probe on each user prompt with a hard ~2 s timeout and, only when a suggestion clears the score floor, injects a one-line `additionalContext` hint (`Relevant skill available: <name> (from the <pack> pack) — view it with: unlimited-skills view <name>`). The hint references skills by NAME only — it never contains local filesystem paths and never echoes the prompt text. It is fail-open by design: any error, timeout, missing CLI, or below-floor result produces no output and exit 0. This converts skill invocation from model initiative into deterministic ambient retrieval.
...
The hooks and router are local-only. The prompt text is passed only to the local CLI for lexical scoring; nothing is uploaded and the plugin makes no network calls. Hosted features remain opt-in through registration as documented in `SECURITY.md`.
