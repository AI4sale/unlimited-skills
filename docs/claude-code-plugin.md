...
- **UserPromptSubmit hook** (`plugin/hooks/user_prompt_submit.py`): runs the `suggest` probe on each user prompt with a hard ~2 s timeout and injects `additionalContext` in three tiers (F3b ambient injection — at high confidence the right skill arrives by itself instead of being hinted at):
