...
## Cross-Repo Readiness Suite

Before v0.4 implementation can start, the Cross-Repo Readiness Suite must prove the public client and private registry contracts work together. The suite runs in public fixture mode and, when available, against a local private registry checkout. It checks signed SkillOps metadata, unsigned and forbidden-field rejection, policy-aware refusal codes, eval gate outcomes, maintainer queue transitions, skill improvement workflow evidence, and support-bundle redaction.

Passing this suite is not a release-owner GO decision. It is technical evidence for the later go/no-go gate and must preserve the no production hosted calls, no production signing key, no live billing, no PyPI publication, no full catalog distribution, no automatic install/update/remove, no automatic skill rewriting, and no auto-publish boundaries.
