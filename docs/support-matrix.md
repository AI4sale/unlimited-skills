...
`v0.2.0-alpha` is a developer preview. Install from a GitHub clone, not PyPI, because repo assets are required.
...
| Vellum AI | Migration script | Not full installer yet | Not yet | Migration-only | Full installer is not implemented in v0.2.0-alpha. |
...
The v0.2.0 alpha distribution path is:
...
PyPI is not the supported v0.2.0 distribution path yet.
