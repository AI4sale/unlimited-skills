...
`v0.1.2-alpha` is a developer preview. Install from a GitHub clone, not PyPI, because repo assets are required.

| Agent | Installer | Router | Patch | Native sync | Notes |
| --- | --- | --- | --- | --- | --- |
| Codex | Yes | Yes | `AGENTS.md` patch yes | Yes | Default installer patches project `AGENTS.md` unless opted out. |
...
| Vellum AI | Migration script | Not full installer yet | Not yet | Migration-only | Full installer is not implemented in v0.1.2-alpha. |
...
The v0.1.2 alpha distribution path is:
...
PyPI is not the supported v0.1.2 distribution path yet.
