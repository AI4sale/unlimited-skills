...
- E25 design (docs/tests only, no implementation, nothing hosted): MCP hosted/team distribution threat model and abuse-case test plan (`docs/mcp-distribution-threat-model.md`, `docs/mcp-distribution-abuse-test-plan.md`, `tests/test_mcp_distribution_threat_docs.py`): the adversarial acceptance criteria for the WHOLE distribution design -- the E23 client side (channels/assignments on the E13-E22 stack) AND the registry-side E24 carrier contract (private repo; cited abstractly via its 20 decisions, reason-code table, and forbidden-field denylist) -- written BEFORE any hosted code exists. The threat model catalogues 23 classes `DT-01`..`DT-23` (consolidating and extending E13 threats 14-18 and E23 threats 19-22), each with attacker position (a fixed 7-position vocabulary: malicious publisher / compromised key / malicious registry / network MitM / malicious teammate / compromised client / insider-operator), description, impact, the EXACT existing mitigation by mechanism (verification step, refusal code `-32014`..`-32019`, schema closure, entitlement gate, registry decision), residual risk, and a fixture-level given/when/then abuse-case test. Coverage includes downgrade-to-unsigned on both sides, malicious-but-validly-signed bundles from legitimate publishers, routing replay and monotonicity bypass, revoked-bundle replay and revoked-while-offline, key-rotation races, channel-name squatting, audience confusion/cross-tenant leaks, assignment conflict manipulation, registry body-exposure and forbidden-field smuggling, anti-oracle probing, entitlement bypass, device-proof replay, publisher role escalation, break-glass abuse, CRL outage exploitation, last-good poisoning, retention-expiry denial, support-bundle leaks, and schema-evolution downgrade -- plus a ranked top-5 residual-risk summary (trusted-publisher betrayal, the compromise-to-revocation window, offline staleness, the host-local E07 boundary, operator availability power). The test plan assigns 40 abuse-case test ids `ABT-NNx` with fixtures (reusing the E18 drill machinery `DrillIssuer`/`ScenarioContext`/scenario builders and the E21 `AcceptanceContext`/`step_*` harness by name), pass criteria, and an owning layer per test (client suite / future registry suite / end-to-end E21-style), a complete threat->mitigation->test->owner traceability table, and a refusal-code coverage table spanning the client family and all 17 E24 registry reason codes. Execution rules are part of every pass criterion: fixture mode only -- ephemeral keys, temp directories, no hosted calls, no network, no production key material. `tests/test_mcp_distribution_threat_docs.py` (16 tests) keeps the two documents consistent: threat ids contiguous and matching across both docs and the traceability table, required fields and vocabularies present, abuse-test ids numbered to their threats and identical in model and plan, every cited numeric code inside `-32001`..`-32019`, every cited registry code in the known E24 set with the coverage table complete in both directions, code-like identifiers resolved to a known family, and no TODO/TBD markers. Append-only cross-references added to `docs/mcp-bundle-distribution.md` and `docs/mcp-incident-runbook.md`. Explicit non-goals: no implementation, no verifier/schema/format changes, no new refusal codes, no production keys, no hosted calls; the `ABT-*` tests land WITH the future implementation, each in its owning layer.
