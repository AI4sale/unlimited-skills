...
- Keep expected runtime startup latency in the non-terminal
  ...
  terminating the attempt as `FAILED_RETRYABLE`.
...
- Advertise the `fleet-payload-v2` and exact rc5 client capabilities so the
...
- v0.5.2-alpha adoption instrumentation release gate: package/plugin versions
  ...
  rollup generator, A3.1.1 local effectiveness v2 instrumentation, and A3.1.1
  privacy-gate follow-up before the `v0.5.2-alpha` tag can be created.
