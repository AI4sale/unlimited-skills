...
- A3.1 one-command Claude Code MCP gateway installer (`unlimited_skills/mcp/install.py`, `tests/test_mcp_install_claude_code.py`): `unlimited-skills mcp install --claude-code [--project | --global] [--dry-run] [--force] [--json]` registers the Unlimited Tools gateway as the `unlimited-tools` server in the same places `mcp savings` reads — `./.mcp.json` (`--project`, the default) or the top-level `mcpServers` of `~/.claude.json` (`--global`). Safety contract: timestamped backup next to the config before ANY write (path printed), idempotent rerun ("already installed", exit 0, no new backup), other servers and unrelated top-level keys never touched, a differing same-named entry refused without `--force` (with `--force`: replaced after backup), an unparseable config always refused — even with `--force` — never silently overwritten, `--dry-run` prints the before/after diff of the one managed entry and writes nothing, every write is atomic (temp file + replace) and JSON-validated before and after, and output (human and `--json`) carries server names and file paths only — configured env/header VALUES are redacted in diffs and never printed. The written entry is pip-clean (`"command": "unlimited-skills"` plus a portable `~/.unlimited-skills/gateway-config.json` reference — no repo or machine paths), a minimal gateway config is created when none exists (an existing one is never modified), and the exact next step (restart the Claude Code session) is printed after install. Companions: `unlimited-skills mcp uninstall --claude-code [--project | --global] [--dry-run] [--json]` removes ONLY the `unlimited-tools` entry (backup first, "not installed" exits 0) and `unlimited-skills mcp install-status --claude-code [--json]` reports both scopes read-only (exit 0 installed / 1 not). `quickstart` next steps and `docs/claude-code-plugin.md` now point at the one command.
