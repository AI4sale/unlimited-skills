...
- A1: `unlimited-skills mcp savings [--json] [--timeout SECONDS] [--claude-config PATH]` (`unlimited_skills/mcp/savings.py`, `tests/test_mcp_savings.py`, `docs/quickstart.md`) measures the user's REAL standing MCP context cost: discovers MCP servers from the user's Claude Code configuration (top-level `mcpServers` in `~/.claude.json`, per-project `mcpServers` sections, and each known project's `.mcp.json`), spawns each stdio server the way the host would (same command/args/configured env, via the gateway's own `UpstreamClient`; `cmd /c` wrappers unwrapped, opaque shell strings refused, per-server timeout default 12 s), measures the full `tools/list` payload in bytes plus an `est_tokens = bytes/4` heuristic estimate, and compares the summed standing cost against the gateway's live-measured 3 meta-tools. Unreachable servers become `skipped (not reachable)` rows, remote (`sse`/`http`) servers are `skipped (remote server; not measured)` — never a failure. With no MCP servers configured the lab benchmark is shown instead (90,420 B full dump vs 1,268 B gateway). Local-only by construction: nothing is uploaded; the report and the numbers-only snapshot appended to `<root>/.learning/events.jsonl` carry only server names, tool counts, byte sizes, and fixed status strings — never schema contents, spawn commands, args, or env names/values.
- A1: `unlimited-skills quickstart [QUERY] [--json] [--skip-mcp-check] [--timeout SECONDS] [--claude-config PATH]` (`unlimited_skills/quickstart.py`, `tests/test_quickstart.py`, `docs/quickstart.md`, README Quickstart section) — the one-command golden path for a fresh install: imports the bundled `ecc` + `superpowers` packs from the repo's `packs/` directory when the library is empty (the installers' existing `migrate_source` bundled path; a populated library is never touched; a packless pip-from-GitHub install gets an `install-pack` hint), runs a first lexical search with the top 3 hits (falling back to a demo query so first-run output is never empty), runs the `mcp savings` measurement as the wow step, and prints the exact next-step commands (`mcp gateway`, `setup --local-only`). Idempotent across reruns; appends a `quickstart` snapshot event to the local learning log.
