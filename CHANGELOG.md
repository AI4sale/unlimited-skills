...
- A0 invocation rescue, F1 — `unlimited-skills suggest "<task>"` (`unlimited_skills/suggest.py`, `unlimited_skills/search_core.py`, `unlimited_skills/__main__.py`, `tests/test_suggest.py`): a fast, always-cheap invocation probe. Lexical-only (never loads embedding models, never syncs native roots), top-3 one-liners `name — description (score)`, machine-friendly `--json`, and a calibrated score floor (default 12.0) below which it prints NOTHING and exits 0 — silence beats noise and means "proceed with the task". The lexical search core moved from `cli.py` into the import-cheap `search_core.py` (re-exported from `cli` for full backwards compatibility), and `python -m unlimited_skills` / the console script / all rendered launchers now dispatch `suggest` WITHOUT importing the heavy CLI module. Measured cold latency on the bundled 267-skill library: p90 ~450 ms direct spawn, ~790 ms through the PowerShell launcher (budget: p90 < 1.5 s; the previous documented router command cost 3.9 s lexical / 9.9 s hybrid per call). Event logging is best-effort (`suggest` events with latency) and can never fail the probe.
