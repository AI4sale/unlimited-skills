...
def _patch_agents_file(agents_file: Path, launcher: Path) -> None:
    ...
    block = "\n".join(
        [
            ...
            "",
            "A local library of 250+ proven skills (checklists, workflows, regression recipes) that are deliberately NOT in the always-loaded skill list. A 1-second lookup often replaces 20 minutes of rediscovery — the library has shipped-and-tested procedures for most recurring tasks.",
            "",
            ...
            "",
            "```bash",
            f"\"{launcher_text}\" suggest \"<task in 3-8 keywords>\"",
            "```",
            "",
