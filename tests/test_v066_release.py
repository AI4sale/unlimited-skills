...
def test_v069rc10_versions_and_v068_history_are_aligned() -> None:
    ...
    assert 'version = "0.6.9rc10"' in pyproject
    assert '__version__ = "0.6.9rc10"' in runtime
    assert plugin["version"] == "0.6.9rc10"
    assert marketplace["plugins"][0]["version"] == "0.6.9rc10"
    assert "**v0.6.9rc10 / pre-1.0.**" in readme
    assert "This is `v0.6.9rc10`" in pypi_readme
    assert "unlimited-skills==0.6.9rc10" in pypi_readme
    assert "### Added (0.6.9 - Fleet Contract Foundation)" in changelog
    assert "`v0.6.9-rc.10`" in changelog
