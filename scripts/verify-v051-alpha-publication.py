"""Verify the v0.5.1-alpha adoption tools publication gate."""
...
VERSION = "0.5.1"
MANIFEST = ROOT / "docs" / "releases" / "v0.5.1-alpha.release-manifest.json"
REQUIRED_DOCS = [
    ROOT / "docs" / "releases" / "v0.5.1-alpha.md",
    ROOT / "docs" / "releases" / "v0.5.1-alpha-checklist.md",
    ROOT / "docs" / "releases" / "v0.5.1-alpha-pypi-publishing.md",
    ROOT / "docs" / "feedback.md",
    ROOT / "docs" / "adoption" / "marketplace-listing-launch-pack.md",
...
def load_package_smoke():
    path = ROOT / "scripts" / "run-v051-alpha-package-smoke.py"
    spec = importlib.util.spec_from_file_location("run_v051_alpha_package_smoke", path)
...
def run_package_smoke() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="uls-v051-verifier-") as tmp_dir:
        tmp = Path(tmp_dir)
        dist_dir = tmp / "dist"
        dist_dir.mkdir()
...
def assert_manifest() -> dict[str, Any]:
    ...
    require(git.get("publication_branch") == "release/v0.5.1-alpha-adoption-tools", "publication branch mismatch")
    ...
    require(119 in excluded, "#119 must be explicitly excluded from v0.5.1")
    return payload
...
def release_blocker(reason: str, details: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        ...
        "action": "Publish unlimited-skills 0.5.1 through the manual Trusted Publishing workflow, then rerun the post-publish smoke.",
        "fallback": "Keep v0.5.1-alpha tag and GitHub prerelease blocked; do not mark the package release as published.",
        "details": details or {},
    }
...
def published_install_smoke() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="uls-v051-pypi-smoke-") as tmp_dir:
        tmp = Path(tmp_dir)
        env_dir = tmp / "venv"
        ...
        return {"ok": True, "version_output": version.stdout.strip()}
