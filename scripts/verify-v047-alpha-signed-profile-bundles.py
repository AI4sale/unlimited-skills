...
def assert_manifest() -> dict[str, Any]:
    ...
    require(
        git.get("publication_branch") == "release/v0.4.7-alpha-signed-profile-bundles-integration",
        "publication branch mismatch",
    )
    require(git.get("tag") == RELEASE, "manifest tag mismatch")
    require(git.get("tag_status") == "not_created_by_codex", "integration gate must not create the v0.4.7-alpha tag")
