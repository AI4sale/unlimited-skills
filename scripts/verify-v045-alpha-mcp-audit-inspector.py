...
def assert_manifest() -> dict[str, Any]:
    ...
    require(git.get("publication_branch") == "release/v0.4.5-alpha-mcp-audit-inspector-integration", "publication branch mismatch")
    require(git.get("tag") == RELEASE, "manifest tag mismatch")
    require(git.get("tag_status") == "not_created_by_codex", "integration gate must not create a tag")
...
def assert_docs() -> None:
    ...
    for required in (
        ...
        "codex must not create or push v0.4.5-alpha",
    ):
