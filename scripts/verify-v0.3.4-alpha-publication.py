...
RELEASE_DOCS = [
    ...
    ROOT / "docs" / "releases" / "v0.3.4-alpha-github-release-draft.md",
    MANIFEST,
]
...
PRIVATE_MATERIAL_PATTERNS = {
    ...
    "invoice_url_field": r'"invoice_url"\s*:',
    "card_number_field": r'"card_number"\s*:',
    "bank_account_field": r'"bank_account"\s*:',
    "private_skill_body": r"(?is)##\s*when to use.+##\s*procedure.+##\s*tools",
}
...
def assert_manifest(payload: dict[str, Any], expected_sha: str | None) -> str:
    ...
    require(git_info.get("publication_branch") == "release/v0.3.4-alpha-final-publication", "manifest publication branch mismatch")
    ...
    require(distribution.get("official_alpha_path") == "GitHub clone", "GitHub clone must be the alpha distribution path")
    require(distribution.get("pypi_supported") is False, "PyPI must remain deferred")

    registration = payload.get("registration_boundary", {}) if isinstance(payload.get("registration_boundary"), dict) else {}
    require(registration.get("local_mit_core_unregistered") is True, "MIT local core must remain unregistered")
    require(registration.get("plan_refresh_registration_gated") is True, "plan refresh must require registration")
    require(registration.get("billing_refresh_registration_gated") is True, "billing refresh must require registration")
    ...
    require(billing.get("card_data_stored") is False, "card data must not be stored")
    require(billing.get("bank_data_stored") is False, "bank data must not be stored")
...
def assert_docs() -> None:
    ...
    for required in (
        ...
        "support bundle",
        "release owner",
        "pending_release_owner_approval",
    ):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify v0.3.4-alpha final publication gate before tagging.")
