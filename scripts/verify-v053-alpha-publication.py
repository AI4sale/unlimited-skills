"""Verify the v0.5.3-alpha local event privacy publication gate."""
...
VERSION = "0.5.3"
MANIFEST = ROOT / "docs" / "releases" / "v0.5.3-alpha.release-manifest.json"
REQUIRED_DOCS = [
    ROOT / "docs" / "releases" / "v0.5.3-alpha.md",
    ROOT / "docs" / "releases" / "v0.5.3-alpha-checklist.md",
    ROOT / "docs" / "releases" / "v0.5.3-alpha-pypi-publishing.md",
    ROOT / "docs" / "adoption" / "local-event-privacy-audit.md",
    MANIFEST,
]
...
def run_package_smoke() -> dict[str, Any]:
    ...
    smoke052 = module.load_v052_smoke()
    smoke050 = smoke052.load_v050_smoke()
    import tempfile

    with tempfile.TemporaryDirectory(prefix="uls-v053-verifier-") as tmp_dir:
        tmp = Path(tmp_dir)
        dist_dir = tmp / "dist"
        dist_dir.mkdir()
        ...
        report = {
            ...
            "clean_install_local_event_privacy": module.clean_install_local_event_privacy_smoke(smoke050, wheel, tmp / "privacy"),
        }
        report["errors"] = module.verify(report, smoke052, smoke050)
...
def assert_manifest() -> dict[str, Any]:
    ...
    require(git.get("publication_branch") == "release/v0.5.3-alpha-local-event-privacy", "publication branch mismatch")
    ...
    require(119 in excluded, "#119 must be explicitly excluded from v0.5.3")
    return payload
...
def release_blocker(reason: str, details: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        ...
        "action": "Publish unlimited-skills 0.5.3 through the manual Trusted Publishing workflow, then rerun the post-publish smoke.",
        "fallback": "Keep v0.5.3-alpha tag and GitHub prerelease blocked; do not mark the package release as published.",
        "details": details or {},
    }
...
def published_install_smoke() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="uls-v053-pypi-smoke-") as tmp_dir:
        tmp = Path(tmp_dir)
        env_dir = tmp / "venv"
        ...
        return {"ok": True, "version_output": checks["version"].stdout.strip()}
